...
out_file = sys.argv[1] + ".nobias.table"
...
for line in content:  # loop through each line of the vcf
    ...
    # eg. uncle='0/1:41,5:46:87:.:.:87,0,1481', uncle[0:3]='0/1'
    ...
    if (uncle[0:3] == '0/1' and S3[0:3] == '0/0' and S4[0:3] == S5[0:3] == S6[0:3] == '1/1') or \
        ...
        sum_ref = 0
        sum_alt = 0
        # polarize uncle read counts first and calculate mapping bias n
        ...
        freq_R = uncle_cnt_ref / float(uncle_cnt_ref + uncle_cnt_alt)
        freq_N = 1 - freq_R
        n = math.sqrt(freq_R * (1 - freq_N) / float(freq_N * (1 - freq_R)))
        ...
        for i in range(9, 22):  # for 7 samples and pool sum2
            ...
            if re.match('0\/0', vcf_line[i]) \
                ...
                if cnt_ref + cnt_alt != 0:
                    allele_freq = cnt_ref * n / float(cnt_ref * n + cnt_alt)
                else:
        ...
        if sum_ref + sum_alt != 0:
            sum_allele_freq = sum_ref * n / float(sum_ref * n + sum_alt)
