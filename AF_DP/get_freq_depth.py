...
out_file = sys.argv[1] + ".depth_table"
f_out = open(out_file, "w")
# print header line of the output
print('#CHROM\tPOS\tpool_1\t1_DP\tpool_2\t2_DP\tpool_3\t3_DP\tpool_4\t4_DP'
      '\tpool_5\t5_DP\tpool_6\t6_DP\tpool_7\t7_DP\tpool_combine\tc_DP\tpool_sum\ts_DP', file=f_out)
...
for line in content:  # loop through each line of the vcf
    ...
    if (uncle[0:3] == '0/1' and S3[0:3] == '0/0' and S4[0:3] == S5[0:3] == S6[0:3] == '1/1') or \
        ...
        for i in range(9, 22):  # for 7 samples and pool combine
            ...
            if re.match('0\/0', vcf_line[i]) \
                ...
                # do not add counts from pool combine!
                if i == 22:
                    cnt_ref, cnt_alt = 0, 0
                sum_ref += cnt_ref
                sum_alt += cnt_alt
            ...
            result_list.append(str(cnt_ref + cnt_alt))
        if sum_ref + sum_alt != 0:
            ...
            result_list.append(str(0))
        result_list.append(str(sum_ref + sum_alt))
